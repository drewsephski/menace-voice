...
Deploy Menace Voice where a shared, managed Traefik with Let's Encrypt already
...
|---|---|---|
| `docker-compose.yaml` | The Menace Voice app stack. **Single self-contained file** — named volumes only, no host bind-mounts, no init/sidecar that reads files outside the compose. | ✅ Yes |
