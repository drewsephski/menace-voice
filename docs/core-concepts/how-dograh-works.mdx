...
sequenceDiagram
    ...
    participant API as Menace Voice API
...
The phone infrastructure. Menace Voice connects to your telephony provider (Twilio, Vonage, etc.) to place and receive calls. The audio streams between the caller and Menace Voice in real time.

**[Transcriber (STT)](/configurations/transcriber)**
Converts the caller's speech to text in real time. Menace Voice sends the audio stream to your configured speech-to-text provider and uses the transcript to drive both the LLM and the final run record.
...
1. Menace Voice instructs your telephony provider to dial the number
...
6. When an edge condition is met, Menace Voice transitions to the next node
