---
title: "Menace Voice Managed SIP"
description: "Connect your own SIP carrier or PBX to the SIP domain Menace Voice provisions for your organization"
---

## What you get, and what you bring

When managed provisioning is available and completes successfully, your organization receives a telephony configuration named **Menace Voice Cloudonix SIP**. It provides a SIP domain connected to the Menace Voice pipeline without requiring your own Cloudonix account. Check that this configuration exists and its setup checklist is complete before relying on managed SIP.

What it does **not** include is phone service. Menace Voice does not sell or resell
phone numbers, and no calls flow until you connect a carrier:

| Menace Voice provides                                     | You provide                                    |
| --------------------------------------------------- | ---------------------------------------------- |
...
<Note>
  Until you finish the steps below, **Menace Voice Cloudonix SIP** cannot place a
...
## Step 1: Read your SIP endpoints from Menace Voice

1. Go to **Telephony configurations** and open **Menace Voice Cloudonix SIP**
...
4. Note the two values you will need on the carrier side:
   - **Inbound hostname** and **ports** — where your carrier sends calls *to* Menace Voice
   - **Outbound origin IP address** — the address Menace Voice's calls arrive *from*

## Step 2: Point your carrier at Menace Voice (inbound)
...
Menace Voice needs to know where to send calls when your agent dials out.

1. On your carrier, allow the **origin IP address** from Step 1 to send SIP
   traffic to your trunk. Calls from Menace Voice are unauthenticated and identified
   by source IP, so this step is required
2. Back in Menace Voice, under **Outbound trunks**, choose **Add trunk** and fill in:
   - **Name** — any name you choose. Letters, digits and hyphens only
   - **SIP domain** — the hostname of your carrier or PBX, for example
     `sip.example.com`. Menace Voice puts this in both the SIP `To` header and the
     Request-URI
   - **Region** — the edge Menace Voice dials your carrier from. The dialog shows
...
rejected by the carrier even though Menace Voice accepts it — which is why the trunk
assignment matters as soon as you have more than one carrier. With a single
trunk, Menace Voice uses it for every number and the field can be left alone.
...
Menace Voice pushes the inbound webhook URL to the Voice Application on the
...
2. Choose **Phone Call**
3. Select **Menace Voice Cloudonix SIP**, pick your caller ID, enter a destination
