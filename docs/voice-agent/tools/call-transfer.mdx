...
When the LLM decides a transfer is needed, it calls the Call Transfer tool. Menace Voice resolves the destination, validates the transfer setup, and then starts the provider transfer.

For a successful transfer:

1. The agent calls the Call Transfer tool.
2. Menace Voice resolves the destination from static configuration, ordered context mappings, or a dynamic HTTP resolver.
3. Menace Voice optionally plays a pre-transfer message.
4. Menace Voice starts dialing the destination through the telephony provider.
5. The caller hears hold music while the destination leg is connecting.
6. When the destination answers, Menace Voice connects the caller and ends the AI agent's involvement.

The transfer is a blind transfer. Menace Voice does not currently pass conversation context to the receiving human as part of the transfer.
...
Use this when a finite set of context values should select the transfer destination. You can add multiple rules and order them. Menace Voice evaluates the rules from top to bottom and uses the first route whose value matches, ignoring case and surrounding whitespace.
...
Menace Voice performs final variable extraction before evaluating context mappings. If no rule matches, it uses the optional fallback destination. If there is no fallback, or a selected destination template resolves to an empty value, the transfer fails before dialing and the agent can continue the call.

### Dynamic HTTP Resolver

Use this when the destination must be decided during the conversation.

Menace Voice sends a `POST` request to your resolver endpoint before starting the transfer. Your endpoint returns the destination and may return a custom message to play before dialing.
...
- Use business logic from your backend instead of hardcoding numbers in Menace Voice

## Dynamic resolver request

The resolver request body is a flat JSON object.

Menace Voice builds it from:

- **LLM parameters**: values the agent extracts from the conversation when it calls the transfer tool
- **Preset parameters**: values Menace Voice injects from fixed values or templates such as `{{initial_context.account_id}}` or `{{gathered_context.billing_issue_type}}`
...
<Note>
Menace Voice does not send the full conversation transcript to the resolver. Add the specific values you need as LLM parameters or preset parameters.
</Note>
...
<Warning>
Follow this response shape exactly. Menace Voice reads specific keys from the resolver response. For dynamic transfer resolution, an invalid response does not fall back silently: the transfer fails before dialing.
</Warning>
...
- **Resolver URL**: HTTPS or HTTP endpoint Menace Voice calls with `POST`
- **Resolver Timeout**: How long Menace Voice waits for the resolver response
- **Resolver Wait Message**: Optional message spoken while Menace Voice waits, such as "One moment while I find the right team."
- **LLM Parameters**: Values the agent should extract from the conversation, such as `billing_issue_type` or `requested_department`
- **Preset Parameters**: Values injected by Menace Voice from context or fixed values
...
1. Menace Voice resolves the static or template destination.
2. Menace Voice plays the configured pre-transfer message, if any.
3. Menace Voice starts the provider transfer.

For dynamic transfers:

1. Menace Voice may play the resolver wait message while calling your resolver.
2. If the resolver returns `custom_message`, Menace Voice plays that message.
3. If the resolver does not return `custom_message`, Menace Voice uses the configured pre-transfer message.
4. Menace Voice starts the provider transfer.

Resolver `custom_message` overrides the configured pre-transfer message for that transfer attempt.

## Transfer timeout

Transfer timeout is the maximum time Menace Voice waits for the destination to answer after the provider transfer starts.

This timeout applies to static, context-mapped, and dynamic transfers.

The resolver timeout is separate. It controls only how long Menace Voice waits for your HTTP resolver before dialing starts.
...
- a preset parameter, if Menace Voice should inject it from context
