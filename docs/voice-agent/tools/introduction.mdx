...
Tools let your AI agent take actions during a conversation — transfer calls, end calls, look up the current time, call external APIs, or invoke remote MCP servers — based on the context of the conversation and your prompt instructions.
...
Menace Voice provides two categories of tools:
...
- [**Current Time**](/voice-agent/tools/current-time) — Look up the current time or convert a time between timezones
