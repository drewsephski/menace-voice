...
Choose whether Menace Voice makes a POST request before **all**, **inbound**, or **outbound** calls, or leave the fetch **disabled**. Menace Voice merges the JSON response into the call context as template variables before rendering the greeting and Start-node prompt.
