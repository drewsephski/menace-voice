...
2. Menace Voice sends a **POST** request to your configured endpoint with a standardized payload.
...
You can also set a lookup URL during [agent onboarding](/getting-started/first-agent) on the Launch step. That writes the same Start Call fields as the node editor.
