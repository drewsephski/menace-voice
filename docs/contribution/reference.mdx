...
The checked-in `.devcontainer/` follows the Dev Containers specification, so it should also work in Cursor, JetBrains IDEs, and other compatible editors. Only the VS Code path is actively tested — if something breaks elsewhere, contact your designated Menace support channel.
...
```bash
    origin    <YOUR_SOURCE_URL> (fetch/push)
    upstream  <UPSTREAM_SOURCE_URL> (fetch/push)
```

<Note>
For Menace Voice changes, push a feature branch to your authorized fork or source remote and open a pull request against `drewsephski/menace-voice:main`. The bootstrap scripts retain upstream Dograh defaults for upstream contribution; inspect the remotes before pushing and select the Menace repository explicitly.
</Note>
