...
API endpoint pages point to exact method/path pairs in the spec. Regenerate the extended catalog after changing the schema:
...
Connect `drewsephski/menace-voice` in the Context7 team's **Docs7** tab, set the production branch to the reviewed documentation branch, and set **Docs path** to `docs`. Enable **Add to Context7**. Use `main` after the documentation change is merged if that is the team's chosen release branch.
