...
async def credentials(
    organization_id: int, source: str | None = None
) -> tuple[str, str]:
    ...
    raise VoiceCloneError(
        ...
        "or ask your administrator to enable hosted voice cloning.",
        409,
    )
...
async def capabilities(organization_id: int) -> VoiceCloneCapabilities:
    ...
    return VoiceCloneCapabilities(
        available=True, message="Record 1–2 minutes in a quiet room for the best match."
    )
...
async def operation_lock(organization_id: int):
    ...
    token = await db_client.claim_configuration_lease(
        organization_id, key, timedelta(minutes=5)
    )
    if token is None:
        raise VoiceCloneError(
            "Another voice operation is running. Please try again shortly.", 409
        )
    try:
        yield
    finally:
...
async def provider_request(
    method: str, path: str, api_key: str, **kwargs
) -> httpx.Response:
    try:
        ...
        raise VoiceCloneError(
            "ElevenLabs did not respond. Check your voice library before retrying creation.",
            502,
        ) from exc
    ...
    raise VoiceCloneError(
        messages.get(
            ...
            "ElevenLabs could not complete this operation. Check your recording and account.",
        ),
        502,
    )
...
async def create_clone(
    *, organization_id: int, user_id: int, name: str, consent: bool, sample: bytes
):
    name = name.strip()
    ...
    if not consent:
        raise VoiceCloneError(
            "Confirm that this is your own voice and you consent to cloning it."
        )
    ...
    async with operation_lock(organization_id):
        if len(await db_client.list_voice_clones(organization_id)) >= 10:
            raise VoiceCloneError(
                "Your organization can keep up to 10 voice clones. Delete an unused voice first.",
                409,
            )
        try:
            ...
            raise VoiceCloneError(
                "The recording took too long to process. Try a shorter recording.", 422
            ) from exc
        response = await provider_request(
            ...
            api_key,
            data={"name": name},
            files={"files": ("sample.wav", audio, "audio/wav")},
        )
        try:
            result = response.json()
            voice_id = result["voice_id"]
            if not isinstance(voice_id, str) or not re.fullmatch(
                r"[A-Za-z0-9_-]{1,128}", voice_id
            ):
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "Voice clone persistence and provider cleanup failed for organization {}",
                    organization_id,
                )
            raise
...
async def preview_clone(clone_id: str, organization_id: int, text: str) -> bytes:
    ...
    async with operation_lock(organization_id):
        ...
        response = await provider_request(
            "POST",
            f"/v1/text-to-speech/{clone.provider_voice_id}",
            api_key,
            params={"output_format": "mp3_44100_128"},
            json={"text": text, "model_id": "eleven_flash_v2_5"},
        )
        if (
            not response.headers.get("content-type", "").startswith("audio/")
...
async def delete_clone(clone_id: str, organization_id: int) -> None:
    async with operation_lock(organization_id):
        ...
        if await db_client.voice_clone_in_use(clone_id, organization_id):
            raise VoiceCloneError(
                "Remove this voice from agent drafts and publish those changes before deleting it.",
                409,
            )
        api_key, _ = await credentials(organization_id, clone.credential_source)
        ...
        await provider_request(
            "DELETE", f"/v1/voices/{clone.provider_voice_id}", api_key
        )
...
async def apply_clone_to_config(
    ...
    if clone.status != "ready":
        raise VoiceCloneError(
            "This voice is not ready. Complete provider verification and preview it first.",
            409,
        )
    if configuration.is_realtime:
        raise VoiceCloneError(
            "Voice cloning needs a pipeline agent. Select pipeline mode in Models first.",
            409,
        )
    api_key, _ = await credentials(organization_id, clone.credential_source)
...
async def assign_clone(
    workflow_id: int, clone_id: str | None, organization_id: int
) -> None:
    from api.services.configuration.ai_model_configuration import (
